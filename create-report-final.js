...
const TASK_ID = '9c3f8d7d-cb36-4950-bcfc-adbb20bca6a8';
...
async function generateReport() {
  ...
  results.forEach((r, idx) => {
    ...
    const euCountries = euRecords.map(rec => rec.country).join(', ') || '-';
    const euCodes = euRecords.map(rec => rec.countryCode).filter(Boolean).join(', ') || '-';
  ...
  detailWs['!cols'] = [
    ...
    { wch: 10 }, { wch: 25 }, { wch: 15 }, { wch: 20 }, { wch: 10 }, { wch: 10 }
  ];
  ...
  const filename = `商标查询结果-50个品牌-最终版-${new Date().toISOString().split('T')[0]}.xlsx`;
